...
namespace Checkout.Tests.Payments
{
    public class IdempotencyTests : ApiTestFixture
    {
        ...
        public async Task CanIdempotentlyRequestPayment()
        {
            ...
            var paymentResponse1 = await Api.Payments.RequestAsync(paymentRequest, idempotencyKey);
            paymentResponse1.ShouldNotBeNull();

            var paymentResponse2 = await Api.Payments.RequestAsync(paymentRequest, idempotencyKey);
        ...
        public async Task CanIdempotentlyCapturePayment()
        {
            ...
            var captureResponse1 = await Api.Payments.CaptureAsync(payment.Id, captureRequest, idempotencyKey);
            captureResponse1.ShouldNotBeNull();

            var captureResponse2 = await Api.Payments.CaptureAsync(payment.Id, captureRequest, idempotencyKey);
        ...
        public async Task CanIdempotentlyVoidPayment()
        {
            ...
            var voidResponse1 = await Api.Payments.VoidAsync(payment.Id, voidRequest, idempotencyKey);
            voidResponse1.ShouldNotBeNull();

            var voidResponse2 = await Api.Payments.VoidAsync(payment.Id, voidRequest, idempotencyKey);
        ...
        public async Task CanIdempotentlyRefundPayment()
        {
            ...
            var refundResponse1 = await Api.Payments.RefundAsync(payment.Id, refundRequest, idempotencyKey);
            refundResponse1.ShouldNotBeNull();

            var refundResponse2 = await Api.Payments.RefundAsync(payment.Id, refundRequest, idempotencyKey);
