...
namespace Checkout.Payments.Hosted
{
    public class HostedPaymentsIntegrationTest : AbstractPaymentsIntegrationTest
    {
        ...
        private async Task ShouldCreateAndGetHostedPayment()
        {
            ...
            var createResponse = await DefaultApi.HostedPaymentsClient().Create(hostedPaymentRequest);
            ...
            var getResponse = await DefaultApi.HostedPaymentsClient().Get(createResponse.Id);
